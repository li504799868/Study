...
public class LinearLayout extends ViewGroup {
    ...
    void measureVertical(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (skippedMeasure || remainingExcess != 0 && totalWeight > 0.0f) {
            // 如果设置了weightSum，忽略计算的totalWeight
            ...
            for (int i = 0; i < count; ++i) {
                ...
                if (childWeight > 0) {
                    ...
                    // 如果使用最大child的高度，并且高度是wrap_content
                    if (mUseLargestChild && heightMode != MeasureSpec.EXACTLY) {
                        // child的高度都设置成最大child高度
                        childHeight = largestChildHeight;
                    } else if (lp.height == 0 && (!mAllowInconsistentMeasurement
                            || heightMode == MeasureSpec.EXACTLY)) {
                        // 如果是6.0 以上，且 高度是固定值或match_parent
                        // 等于分配的高度
                        childHeight = share;
                    } else {
                        // 其他情况等于自己的最小高度+分配高度
                ...
                // 更新最大宽度
                maxWidth = Math.max(maxWidth, measuredWidth);
                // 如果宽度是wrap_content或者不限制，child是math_parent
                boolean matchWidthLocally = widthMode != MeasureSpec.EXACTLY &&
                        lp.width == LayoutParams.MATCH_PARENT;
                // 计算最大宽度
                alternativeMaxWidth = Math.max(alternativeMaxWidth,
                        matchWidthLocally ? margin : measuredWidth);
                // 如果所有的child都是match_parent
                allFillParent = allFillParent && lp.width == LayoutParams.MATCH_PARENT;
            ...
            // 如果使用最大child的高度 且 高度是wrap_content
            if (useLargestChild && heightMode != MeasureSpec.EXACTLY) {
                for (int i = 0; i < count; i++) {
                    ...
                    // 如果使用了weight，设置child的高度等于最大child的高度
                    if (childExtra > 0) {
                        child.measure(
        ...
        // 如果不是所有的child都是match_parent ， 且自己是wrap_content或者不限制
        // 最大宽度等于没有使用weight的child的最大宽度
        ...
        // 对比计算的最大宽度 和 推荐的最小宽度
        // 背景的最小宽度和minWidth去最大值
        maxWidth = Math.max(maxWidth, getSuggestedMinimumWidth());

        // 设置宽高
        setMeasuredDimension(resolveSizeAndState(maxWidth, widthMeasureSpec, childState),
                heightSizeAndState);
        // 把使用match_parent的child的宽度设置成最大宽度
        if (matchWidth) {
    ...
    void layoutVertical(int left, int top, int right, int bottom) {
        ...
        // child的right减去paddingRight
        int childRight = width - mPaddingRight;

        // Space available for child
        // child的实际占用的空间，等于自己减去左右padding
        ...
        switch (majorGravity) {
            case Gravity.BOTTOM:
                ...
                // 计算初始的to
                // 因为mTotalLength中包含了paddingTop，所以需要加上paddingTop
                // bottom - top = 自身的高度
                // mTotalLength是child的总高度
                childTop = mPaddingTop + bottom - top - mTotalLength;
                break;

            // mTotalLength contains the padding already
            case Gravity.CENTER_VERTICAL:
                // 计算初始的top，从中心开始，这里加上了上padding
                // 请注意这里没有减去paddingBottom
                // 所以如果paddingTop和paddingBottom不相同，可能会影响居中的效果
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child == null) {
                ...
                // 如果child没有设置gravity，使用自身的gravity
                if (gravity < 0) {
                    gravity = minorGravity;
                }
                // layout的方向，LTR，RTL
                final int layoutDirection = getLayoutDirection();
                // 根据layout的方向，计算实际使用gravity
                ...
                // child之前有divider分割线，childTop要加上分割线的高度
                if (hasDividerBeforeChildAt(i)) {
                ...
                // 更新childTop，加上child的高度和下边距和偏移值，下一个child会摆放在这个child的下面
